...
def resolve_structured_intent(
    ...
    if mode == 'taskboard':
        if action == 'create':
            return DetectedIntent(action='create', target='task', parameters={'addComment': True}, confidence=1.0)
        if action == 'search':
            return DetectedIntent(action='view', target='task', parameters={}, confidence=1.0)
        # default: search/ask about tasks
...
설명 업데이트 예시:
- "설명이 너무 부실하다. 실제로는 sendresult2 API가..." → action: "update", target: "task", parameters: {"description": "사용자가 제공한 상세 설명 내용"}
- "상세 내용을 보강해줘. 이 태스크는 실제로 서버 이전 작업이다..." → action: "update", target: "task", parameters: {"description": "보강된 설명 내용"}
- "설명에 데드라인 정보 추가해줘. 2월 28일까지다" → action: "update", target: "task", parameters: {"description": "기존 설명 + 데드라인 정보", "dueDate": "2026-02-28"}
    ...
    try:
        ...
        # 수정 요청
            ...
            if task:
                ...
                # 설명 변경 (LLM 보강)
                if params.get("descriptionEnrich"):
                    logger.info(f"[DESC-ENRICH] Starting enrichment, original_message length={len(original_message)}")
                    enriched = False
                    try:
                        enrich_handler = get_llm_handler()
                        enrich_prompt = f"""기존 태스크 제목: {task.title}
기존 태스크 설명:
...
위 내용을 종합하여 주간보고 스타일의 전문적인 태스크 설명을 마크다운 형식으로 작성하세요.
...
규칙:
- 마크다운 텍스트만 반환 (JSON이 아님, 코드블록으로 감싸지 마세요)
- 기존 설명의 핵심을 유지하면서 사용자 정보를 통합
- 불필요한 섹션은 생략 가능하지만 **개요**, **상세 내용**, **요청 사항**은 필수"""

                        enrich_resp = await enrich_handler.simple_chat(
                            prompt=enrich_prompt,
                            system_prompt="당신은 업무 태스크 설명을 주간보고 형식으로 체계적으로 정리하는 전문가입니다. 마크다운 형식으로 깔끔하게 작성하세요.",
                            temperature=0.3,
                            max_tokens=1500,
                            call_type="description_enrichment",
                        )
                        result_text = enrich_resp.content.strip()
                        # 코드블록으로 감싸진 경우 제거
                        ...
                        if len(result_text) > 30:
                            task.description = result_text
                            enriched = True
                            _append_activity_log(task, "설명 보강", "AI가 사용자 요청에 따라 설명을 보강")
                            logger.info(f"[DESC-ENRICH] description enriched, length={len(task.description)}")
                        else:
                            logger.warning(f"[DESC-ENRICH] LLM response too short ({len(result_text)}), falling back")
                    except Exception as e:
                        logger.warning(f"설명 보강 실패: {e}")

                    # LLM 실패 시 fallback: 기존 설명 + 사용자 메시지를 수동 구조화
                    if not enriched:
                        fallback_desc = f"**개요**\n{task.description or task.title}\n\n**상세 내용**\n{original_message}"
                        task.description = fallback_desc
                        _append_activity_log(task, "설명 보강", "사용자 요청에 따라 설명을 보강 (수동)")

                    # 사용자 메시지에서 마감일 자동 추출
                    if "dueDate" not in params:
                        import re
                        date_match = re.search(r'(\d{1,2})월\s*(\d{1,2})일', original_message)
                        if date_match:
                            month = int(date_match.group(1))
                            day = int(date_match.group(2))
                            year = datetime.utcnow().year
                            try:
                                parsed_due = datetime(year, month, day)
                                task.due_date = parsed_due
                                _append_activity_log(task, "마감일 변경", f"AI가 마감일을 '{month}월 {day}일'(으)로 변경")
                            except ValueError:
                                pass
...
async def process_chat_message(
    ...
    # 3.2. 설명 업데이트 키워드 오버라이드 - LLM이 description을 파라미터에 누락하는 경우 보완
    if context and context.type == 'task' and context.id:
        is_desc_update = any(kw in content for kw in ["설명", "상세", "내용", "부실", "생략", "보완", "추가해", "업데이트"])
        has_detail = len(content) > 50  # 충분한 내용이 포함된 메시지
        if is_desc_update and has_detail and intent.action in ("update", "chat"):
            # LLM을 사용하여 기존 설명 + 사용자 메시지로 풍부한 설명 생성
            intent = DetectedIntent(
                action="update",
                target="task",
                parameters={**intent.parameters, "descriptionEnrich": True},
                confidence=max(intent.confidence, 0.8),
            )
            logger.info(f"[DESC-OVERRIDE] descriptionEnrich=True injected for description update")
