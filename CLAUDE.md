...
### 5.1 재료 관리 (대부분 CLI 전용 — 지식만 웹 편집 허용)

> **편집 정책 (2026-05-14 갱신):** 노드/API명세/워크플로우/인스턴스DB 메타는 **CLI 전용**. 지식문서(`/knowledge`)는 **웹 UI에서도 인라인 편집·삭제·등록 허용**. 지식은 복잡한 로직 없는 순수 데이터 덩어리이며, 코드리뷰 지표 등 점진적 큐레이션이 필요하기 때문.
