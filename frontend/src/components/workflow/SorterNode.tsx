...
function SorterNodeInner({ data, selected, id }: { data: SorterNodeData; selected: boolean; id: string }) {
  ...
  // Total handles: one per rule + one default
  ...
  return (
    <div
      className={`bg-gradient-to-b ${isInvalidTarget ? 'from-red-800/80 to-red-900/80 border-red-500' : 'from-violet-700 to-violet-900 border-violet-400'} border-2 rounded-xl shadow-2xl min-w-[220px] transition-all relative ${
    ...
    >
      ...
      <div className="px-4 py-3 space-y-2">
        ...
        <div className="flex items-center gap-1.5 pt-1 border-t border-white/10">
          ...
          <span className="text-violet-300/60 text-[10px]">
            {itemCount > 0 ? `${itemCount}개 처리됨` : '처리 대기 중'}
          </span>
        </div>
      </div>
      ...
      {/* Default (기타) handle at bottom of right side */}
      <div
        style={{
          position: 'absolute',
          right: 22,
